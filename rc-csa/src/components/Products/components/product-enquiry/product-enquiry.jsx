...
import './product-enquiry-module.css';
import { useApplicationContext } from '@commercetools-frontend/application-shell-connectors';
...
const ProductsEnquiry = (props) => {
  ...
  const [productSearchResults, refreshResults] = useReducer(
    getProductItemsRows,
    null
  );
  ...
  useEffect(async () => {
    if (categoriesMap === null) {
      ...
      FACETS_KEY_VALUE_MAP.find((f) => f.key === 'categories.id').values =
        catMap;
    }
    await search('');
  }, []);
  ...
  const search = async (text, queryFilter, isReset) => {
    const { data } = await executeSearch(
      text,
      dataLocale,
      FACETS_KEY_VALUE_MAP.map((f) => f.key),
      queryFilter,
      page,
      perPage
    );
    ...
    refreshResults({
      productProjectionSearch: data?.productProjectionSearch,
      dataLocale,
      currencyCode: 'USD',
    });
    // getFacets({facetsFromSearch:data?.productProjectionSearch?.facets});
    console.log('better work', data?.productProjectionSearch);
    if (data?.productProjectionSearch?.facets) {
      setWidthSearch('85%');
    }

    const fCheckBoxes = await getFacetsResults(
      data?.productProjectionSearch?.facets,
      facetsCheckboxes,
      isReset
    );
    setFacetsCheckboxes({
            ...
            <tr>
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          {facetsCheckboxes[facetCheckbox].terms.map(function (
                            facetTerm
                          ) {
                            return (
                              <>
                                <CheckboxInput
                                  ...
                                  key={facetTerm.value}
                                  onChange={(event) => {
                                    const { fCheckboxes, queryFilters } =
                                      applyFacetFilter(
                                        facetsCheckboxes,
                                        facetTerm
                                      );
              ...
              <td width={widthSearch}>
                <Spacings.Stack scale="l">
                  <DataTable
                    isCondensed
                    rows={productSearchResults.slice(0, 3)}
                    columns={columns}
                    maxHeight={600}
                    key="table"
                    onRowClick={(row) => push(`../product-enquiry/${row.id}`)}
                  />
...
ProductsEnquiry.displayName = 'ProductsEnquiry';
// Products.propTypes = {
