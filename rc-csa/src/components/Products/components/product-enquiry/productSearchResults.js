...
export function getProductItemsRows(
  state,
  { productProjectionSearch, dataLocale, currencyCode }
) {
  if (productProjectionSearch?.results) {
    let returnValue = productProjectionSearch?.results.map((item) => {
      ...
      price = new Intl.NumberFormat(dataLocale, {
        style: 'currency',
      ...
      let quantity =
        item?.masterVariant?.availability?.noChannel?.availableQuantity;
      if (quantity == null) {
        quantity = 0;
      }
      return {
...
export async function getFacetsResults(
  facetsResults,
  facetsCheckboxes,
  isReset
) {
...
export const applyFacetFilter = (facetsCheckboxes, facetTerm) => {
  ...
  for (const chkBoxKey of Object.keys(facetsCheckboxes)) {
    ...
    var queryFilter = {};
    queryFilter.values = chkBox.terms
      .filter((t) => t.checked)
      .map((t1) => `\"${t1.value}\"`)
      .join(',');
