...
export const useGetTicketByCustomerEmail = () => {
  const [customObjects, { loading }] = useMcLazyQuery(
    gql`
      ${FETCH_CUSTOMER_TICKETS}
    `
  );
...
export const useFetchFeedbackList = () => {
  const dispatch = useAsyncDispatch();
  const { ctCsaBackendURL } = useApplicationContext((context) => ({
    ctCsaBackendURL: context.environment.CT_CSA_BACKEND,
  }));

  const apiUrl = ctCsaBackendURL + '/feedback-list';
...
export const useCreateOrUpdateFeedback = () => {
  const dispatch = useAsyncDispatch();
  const { ctCsaBackendURL } = useApplicationContext((context) => ({
    ctCsaBackendURL: context.environment.CT_CSA_BACKEND,
  }));
