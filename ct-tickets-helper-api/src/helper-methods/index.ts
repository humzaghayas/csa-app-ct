...
export async function getCreateTicketDraft(ticketInfo) {
  ...
  const filesStr = ticketInfo.files
    .map((f) => {
  ...
  const commentsStr = ticketInfo?.comments
    ?.map((c) => {
      if (c.createdAt) {
  ...
  if (
    ticketInfo.category &&
    (ticketInfo.category == CONSTANTS.TICKET_TYPE_ORDER_INQUIRY ||
